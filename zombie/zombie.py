# Simulates carrot, iron, and potato drops from zombies
import argparse
...
def simulate_zombie(world_seed: int, max_drops, target_item: str):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Zombie rare drop simulation)")
    parser.add_argument("seed", type=int)
    parser.add_argument("-d", type=int, default=MAX_SEQUENCE)
    parser.add_argument("-n", type=str, required=True, choices=["iron","carrot","potato"])
    args = parser.parse_args()

    seq = simulate_zombie(args.seed, args.d, args.n)

    print(f"Seed {args.seed} {args.n} drops: {seq}")