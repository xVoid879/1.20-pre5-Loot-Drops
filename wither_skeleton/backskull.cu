// !nvcc -O3 -arch=sm_75 backskull.cu -o backskull
#include <cstdio>
#include <cstdint>
#include <inttypes.h>
...
#define SKULL_MD5_0     0x5a9fe82fe5b3dea9ULL
#define SKULL_MD5_1     0x44ac0b3acb5ebf34ULL
#define STAFFORD_MIX_1  0xbf58476d1ce4e5b9ULL
#define STAFFORD_MIX_2  0x94d049bb133111ebULL
#define MIN_SEQUENCE    7       // minimum streak required
...
struct Xoroshiro128Plus {
    ...
    __device__ __forceinline__ Xoroshiro128Plus(uint64_t seed) {
        ...
        l = mix_stafford(unmixed_lo ^ SKULL_MD5_0);
        h = mix_stafford(unmixed_hi ^ SKULL_MD5_1);
    }
    ...
    __device__ __forceinline__ bool next_has_skull() {
        next_int32();
        next_int32();
        return next_float24() < 0.025f;
    }
...
int main() {
    uint64_t start = 15000000000000ULL;  // start
    uint64_t end   = 15000000000001ULL;  // end
