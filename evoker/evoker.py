...
EVOKER_MD5_0 = 0x408a07c01b664e6f
EVOKER_MD5_1 = 0xb184bd0563136d14
...
def simulate_evoker(world_seed: int, max_seq: int = MAX_SEQUENCE):
    rng = LootTableRNG(EVOKER_MD5_0, EVOKER_MD5_1)
    rng.set_seed(world_seed)

    seq = []
    for _ in range(max_seq):
        seq.append(rng.next_int(2))
    return seq

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evoker drop")
    parser.add_argument("seed", type=int)
    parser.add_argument("-d", type=int, default=MAX_SEQUENCE)
    args = parser.parse_args()

    seq = simulate_evoker(args.seed, args.d)
    print(f"Seed {args.seed} evoker emerald sequence: {seq}")
