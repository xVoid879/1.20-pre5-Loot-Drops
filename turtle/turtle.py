...
TURTLE_MD5_0 = 0x90c6e96b8ad33bf3
TURTLE_MD5_1 = 0xbeb0c424a4f8da37
...
def turtle_seagrass_sequence(seed, max_seq=MAX_SEQUENCE):
    # hash hash hash hash
    ...
    for _ in range(max_seq):
        drops = java_next_int(state, 3)  # 0-2
        sequence.append(drops)

    return sequence

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Turtle seagrass")
    parser.add_argument("seed", type=int)
    parser.add_argument("-d", type=int, default=MAX_SEQUENCE)
    
    args = parser.parse_args()

    seq = turtle_seagrass_sequence(args.seed, args.d)
    print(f"Seed {args.seed} turtle seagrass drops: {seq}")
