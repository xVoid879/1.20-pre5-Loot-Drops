...
SHEEP_MD5_0 = 0x623ad1eb5dc6e89e
SHEEP_MD5_1 = 0x0d12c68897c5aba8
...
def simulate_sheep(world_seed: int, max_seq: int = MAX_SEQUENCE):
    rng = LootTableRNG(SHEEP_MD5_0, SHEEP_MD5_1)
    rng.set_seed(world_seed)

    seq = []
    for _ in range(max_seq):
        mutton_count = rng.next_int(2) + 1
        seq.append(mutton_count)
    return seq

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sheep drop")
    parser.add_argument("seed", type=int)
    parser.add_argument("-d", type=int, default=MAX_SEQUENCE)
    args = parser.parse_args()

    seq = simulate_sheep(args.seed, args.d)
    print(f"Seed {args.seed} sheep mutton sequence: {seq}")
