...
PIG_MD5_0 = 0x8cf80f9c53f2e152
PIG_MD5_1 = 0x125638e263910cdb
...
def simulate_pig_drops(world_seed: int, max_seq: int = MAX_SEQUENCE):
    rng = LootTableRNG(PIG_MD5_0, PIG_MD5_1)
    rng.set_seed(world_seed)

    seq = []
    for _ in range(max_seq):
        seq.append(1 + rng.next_int(3))  # 1 to 3
    return seq

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pig drop")
    parser.add_argument("seed", type=int)
    parser.add_argument("-d", type=int, default=MAX_SEQUENCE)
    args = parser.parse_args()

    seq = simulate_pig_drops(args.seed, args.d)
    print(f"Seed {args.seed} pig porkchop sequence: {seq}")
