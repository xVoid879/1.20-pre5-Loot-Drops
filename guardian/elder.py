...
ELDER_MD5_0 = 0xb29ffa7d6b2d2181
ELDER_MD5_1 = 0x23990926ca2a6adf
...
def simulate_elder(world_seed: int):
    rng = LootTableRNG(ELDER_MD5_0, ELDER_MD5_1)
    ...
    for i in range(MAX_SEQUENCE):
        ...
        shards = roll(rng, 0, 2)
        if shards > 0:
            drops.append(("prismarine_shard", shards))

        roll2 = rng.next_int(6)
        if roll2 < 3:
            drops.append(("cod", 1))
        elif roll2 < 5:
            drops.append(("prismarine_crystals", 1))

        drops.append(("wet_sponge", 1))
        ...
        trim_roll = rng.next_int(5)
        if trim_roll == 4:
            drops.append(("tide_armor_trim", 1))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Elder Guardian Drops")
    parser.add_argument("seed", type=int)
    args = parser.parse_args()

    results = simulate_elder(args.seed)
