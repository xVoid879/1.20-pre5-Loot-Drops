...
def next_blaze_rod(state) -> int:
    ...
    extra = round(next_float(state) * looting)
    return set_count + extra


def simulate(world_seed: int, total_kills: int = MAX_SEQUENCE):
    state = get_random_sequence_xoro(world_seed, BLAZE_MD5_0, BLAZE_MD5_1)
    rod_counts = []
    for _ in range(total_kills):
        rod_counts.append(next_blaze_rod(state))
    return rod_counts


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Blaze Rod Drop Sequence (Looting III)")
    parser.add_argument("seed", type=int, help="World seed")
    parser.add_argument("-k", "--kills", type=int, default=MAX_SEQUENCE, help="# of Blaze kills to simulate")
    args = parser.parse_args()

    seq = simulate(args.seed, args.kills)
    print(f"Seed {args.seed} blaze rod drop sequence (Looting III): {seq}")