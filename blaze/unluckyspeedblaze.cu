// !nvcc -O3 -arch=sm_75 unluckyspeedblaze.cu -o unluckyspeedblaze
#include <cstdio>
#include <cstdint>
#include <inttypes.h>
...
#define TOTAL_RODS      6         // we want it so we want 6 in total
#define MIN_KILLS       60       // if = or more, print the seed.

struct Xoroshiro128Plus {
    uint64_t l, h; // state (l and h)
    ...
    __device__ __forceinline__ uint64_t next_long() { // state l and h into 64 bit #
        uint64_t s0 = l;
        uint64_t s1 = h;
...
__global__ void search(uint64_t start, uint64_t end) {
    ...
    for (uint64_t seed = start + idx; seed < end; seed += stride) {
        ...
        while (rods < TOTAL_RODS) {
            kills++;
            if (rng.next_blaze_rod()) {
                rods++;
            }
        }

        if (kills >= MIN_KILLS) {
            printf("Seed %" PRIu64 " needed %d kills\n", seed, kills);
        }
    }
}
...
int main() {
    ...
    int threads = 256; // some reason on colab
    int blocks  = 256; // doesn't let u run this if lower than 70gb disk
